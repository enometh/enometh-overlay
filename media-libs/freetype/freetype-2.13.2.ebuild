...
#
#   Time-stamp: <>
#   Touched: Fri Dec 11 07:07:34 2020 +0530 <enometh@net.meer>
#   Bugs-To: enometh@net.meer
#   Status: Experimental.  Do not redistribute
#   Copyright (C) 2020-2023 Madhu.  All Rights Reserved.
#
# ;madhu 201211 2.10.4 - from 9999 git
# ;madhu 210728 2.11.0 git VER-2-11-0-5-gba47af32b
# ;madhu 210923 2.11.0-r1  VER-2-11-0-93-g6d19b29fb (+ consolas patch)
# ;madhu 220508 2.12.1 VER-2-12-1
# ;madhu 230830 2.12.1->2.13.2 VER-2-13-2-6-gdd1ced4ee3, doc + USE_GIT=true makes and installs refdoc.  ebuild hardcodes python3.9, build docs from freetype-docs, make "prepare, configure" restartable, cannot conditionalise python-single-r1 on IUSE=doc

EAPI=8

PYTHON_COMPAT=( python3_{9..12} )

inherit autotools flag-o-matic libtool multilib-minimal toolchain-funcs python-single-r1
...
src_prepare() {
	...
	# bug #869803
	rm -fv docs/reference/sitemap.xml.gz #;madhu 230830 - NOT IN USE_GIT=true || die
...
multilib_src_compile() {
	...
	if [[ ${PV} == 9999 ]] || ${USE_GIT} ; then
		if multilib_is_native_abi && use doc ; then
			einfo "Building docs"
			sed -i -e "s|^\(PYTHON := \).*$|\1${EPYTHON}|g" ${BUILD_DIR}/unix-def.mk
			emake refdoc
			rm -fv ${BUILD_DIR}/docs/reference/sitemap.xml.gz
		fi
	fi
}
...
multilib_src_install() {
	...
	if multilib_is_native_abi && use doc ; then
		einfo "Installing refdoc"
		docinto html
		dodoc -r ${BUILD_DIR}/docs/reference/*
	fi
...
multilib_src_install_all() {
	...
	if [[ ${PV} != 9999 ]] && ! ${USE_GIT} && use doc ; then
		docinto html
		dodoc -r docs/*
	fi
