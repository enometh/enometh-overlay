...
LICENSE="GPL-2"
SLOT="0"
IUSE="+client cpu_flags_x86_aes cpu_flags_x86_ssse3 +monitor ofono selinux standalone systemd wired external-ell debug"
...
[[ -z "${ELL_REQ}" ]] || DEPEND+=" external-ell? ( ~dev-libs/ell-${ELL_REQ} )"
...
src_configure() {
	...
	if 	[[ ${PV} == *9999* ]]; then
		:
	elif use external-ell; then
	  myeconfargs+=(--enable-external-ell)
	else
		:
	fi
	econf "${myeconfargs[@]}"
