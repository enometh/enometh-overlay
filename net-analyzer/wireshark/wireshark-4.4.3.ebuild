...
#
#   Time-stamp: <>
#   Touched: Tue Jan 21 10:54:49 2025 +0530 <enometh@net.meer>
#   Bugs-To: enometh@net.meer
#   Status: Experimental.  Do not redistribute
#   Copyright (C) 2025 Madhu.  All Rights Reserved.
#
# ;madhu 250121 4.4.3 - bring back support for qt5 (via +"qt5" instead # of "gui" for qt6), verify-sig unsupported. skip release notes # redirection to website nonsense. (if anything it can be copied from # doc/release-notes.ado). nghttp3 needs updating, fix cmake warnings (shown in commit msg), don't use flag zlib for zlib-ng, this is deception, gentoo should not be pushing zlib-ng by overloading the zlib use flag.
...
src_configure() {
	...
	mycmakeargs+=(
		...
		-DENABLE_ZLIBNG=$(usex zlib-ng)
		-DENABLE_ZSTD=$(usex zstd)
	)

		if use gui || use qt5; then
			mycmakeargs+=( -DBUILD_wireshark=yes )
		else
			mycmakeargs+=( -DBUILD_wireshark=no )
		fi
...
src_install() {
	...
	#;madhu 250121 skip this nonsense. one should just copy doc/
	if false; then
	if ! use doc; then
