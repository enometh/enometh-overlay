...
#
#   Time-stamp: <>
#   Touched: Sun Jan 10 17:06:04 2021 +0530 <enometh@net.meer>
#   Bugs-To: enometh@net.meer
#   Status: Experimental.  Do not redistribute
#   Copyright (C) 2021 Madhu.  All Rights Reserved.
#
# ;madhu 210110 1.48.0 - patch install.sh and ebuild with cp
# ;--preserve=timestamps so it preserves timestamps.
#
# x86_64 only bin-packages. binbootstrapping rust is a crock because the
# bootstrap script the earlier version of rust and cannot use the
# installed version.
#
# ;madhu 210824 1.54
# ;madhu 220311 1.59
...
#
# ;madhu 260127 1.92.0 Limited Edition: ALTSRC=true. see BOOT_DATE BOOT_PV, installs rust rust-std and cargo to ${EPREFIX}/opt/rust-{PV}, and does nothing else.

EAPI=8
LLVM_COMPAT=( 21 )
LLVM_OPTIONAL="yes"

ALTSRC=true  # use the tarballs used to bootstrap the next version
BOOT_DATE=2025-12-11
BOOT_PV=1.92.0

inherit edo prefix rust-toolchain toolchain-funcs verify-sig multilib-minimal
inherit optfeature
# inherit llmv-r1 multilib
...
if ! ${ALT_SRC}; then
SRC_URI="abi_x86_64? ( elibc_glibc? ( https://static.rust-lang.org/dist/rust-${PV}-x86_64-unknown-linux-gnu.tar.xz ) )
  rust-src? ( ${MY_SRC_URI} )"
else
	SRC_URI="
...
VERIFY_SIG_OPENPGP_KEY_PATH="${EPREFIX}/usr/share/openpgp-keys/rust.asc" # crock
...
altsrc_native_abi_install() {
	for i in cargo rust-std rustc; do
		pushd ${WORKDIR}/$i-${BOOT_PV}-x86_64-unknown-linux-gnu || die
#			--components="${i}" \
		./install.sh \
			--verbose \
			--disable-verify \
...
multilib_src_install() {
	if multilib_is_native_abi; then
		...
		cp  --preserve=timestamps -vr "${WORKDIR}/rust-${PV}-${rust_target}/rust-std-${rust_target}/lib/rustlib/${rust_target}"\
			"${ED}/opt/${P}/lib/rustlib" || die
	fi

	# BUG: installs x86_64 binary on other arches
	rm -fv "${ED}/opt/${P}/lib/rustlib/"*/bin/rust-llvm-dwp || die
}
