...
#
#   Time-stamp: <>
#   Touched: Fri Dec 11 07:46:12 2020 +0530 <enometh@net.meer>
#   Bugs-To: enometh@net.meer
#   Status: Experimental.  Do not redistribute
#   Copyright (C) 2020 Madhu.  All Rights Reserved.
#
# ;madhu 200617 n4.4-dev-159-g0b3bd001ac
# ;madhu 201211 4.3.1 from git n4.4-dev-2307-g43b4c66e6b
# ;madhu 210228 4.3.2 from git n4.4-dev-3242-g85ab9deb98
# ;madhu 210325 4.4.0 from git n4.4-dev-3625-ga4e518c321 (pulse)
# ;madhu 210614 4.5.0 from n4.5-dev-1103-gdd770883e9 avresample gonn
# ;madhu 210614 4.4.0-r1 - remotes/origin/release/4.4 n4.4-19-g8d172d9409
# ;madhu 210901 4.5.0-r1 n4.5-dev-1822-gfea4f953b5
# ;madhu 220221 5.1 n5.1-dev-608-g0aa7142442
# ;madhu 230908 6.0-r6 n6.1-dev-2155-g86bd0f08d3 booboo, rsvg only on amd64/rust), yasm fails on ccache.conf hard-link,inode-cahce=true, fix configure test for openal, won't build chromium. flto bug fix fails because filter-lto already removes the flag so x86_64 configure never adds --enable-lto, Use a persistent variable or just USE=lto instead. fix binutils-2.41 openal by forcing -fuse-ld=gold
...
# ;madhu 230911 solve the problem without IUSE=lto
FFMPEG_USE_LTO=false
...
src_prepare() {
	...
	#;madhu 230911
	sed -i -e 's|"-lOpenAL32"||g' configure || die

# ;madhu 230911 - no chromium patch
#	echo 'include $(SRC_PATH)/ffbuild/libffmpeg.mak' >> Makefile || die

#;madhu 230911 - disable the test for openal rather than switching to gold
	if use openal; then
	   sed -i  -e 's/^\(enabled.openal.*||\)/\1 true ||/g' configure || die
	fi
}
...
#	[[ ${ABI} == x86 ]] && myconf+=( --disable-x86asm ) # old asm
#	[[ ${ABI} == x86 ]] && myconf+=( --disable-librsvg ) # no rust on x86_32
	...
	# ;madhu 23091  filter-lto unconditionally removes -flto and the check is-flagq check wont for the x86_64
	# ;madhu 23091  filter-lto unconditionally removes -flto and the check is-flagq check wont for the x86_64
	is-flagq "-flto*" && FFMPEG_USE_LTO=true
	filter-lto

	if [[ ABI != x86 ]] && ${FFMPEG_USE_LTO} ; then
		myconf+=( --enable-lto )
	fi

	#;madhu 230911 cannot compile configure test for openal with ld from binutils-41.2: non-canonical reference to canonical protected function `alGetError' in /usr/lib64/libopenal.so, ld: failed to set dynamic section sizes: bad value, force -fuse-ld=gold. gcc -mno-direct-extern-access fixes the configure check_lib test program, but fails in various ways.
	# if configure is not patched to skip the openal test, using gold will make the test pass.

#	if false &&  use openal; then
#		if tc-ld-is-lld || tc-ld-is-gold; then
#			:
#		else
