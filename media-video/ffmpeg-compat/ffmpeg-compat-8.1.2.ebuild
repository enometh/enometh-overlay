...
# ;madhu 230908 6.0-r6 n6.1-dev-2155-g86bd0f08d3 booboo, rsvg only on amd64/rust), yasm fails on ccache.conf hard-link,inode-ccache=true, fix configure test for openal, won't build chromium. flto bug fix fails because filter-lto already removes the flag so x86_64 configure never adds --enable-lto, Use a persistent variable or just USE=lto instead. fix binutils-2.41 openal by forcing -fuse-ld=gold
# ;madhu 240827 n7.1-dev-2312-g7e4784e40c fix openal configure test kludge, no soc patches
# ;madhu 241205 6.1.2 n6.1.2-15-ga6a07eebdf - use ABI_X86="64" MULTILIB_ABIS=x86_64 FEATURES=-ccache, egit repo override branch release/6.1
# ;madhu 260629 8.2.1 - get rid of gentoo verify-sigs release/6.2
...
#	in_iuse soc && use soc && PATCHES+=( "${DISTDIR}"/${FFMPEG_SOC_PATCH} )
	...
	if use openal; then
		...
		sed -i -e 's|"AL/al\.h" alGetError|math.h modf|g' configure || die
		sed -i -e "s|'AL/al\.h' alGetError|math.h modf|g" configure || die
	fi
