...
#
#   Time-stamp: <2022-02-25 16:26:44 IST>
...
if  ${USE_GIT} || [[ ${PV} == *9999 ]] ; then
	...
	if ${USE_GIT}; then
	EGIT_BRANCH="maint"
	EGIT_COMMIT=4c53a8c20f8984adb226293a3ffd7b88c3f4ac1
	EGIT_SUBMODULES=()
	EGIT_MANPAGES_COMMIT=2d0ff4c32967fbcef181a4565fca8dee05e40485
...
# at update-manifest time
	...
	SRC_URI="${SRC_URI_KORG}/${MY_P}${EXPORT_SUFFIX}.tar.${SRC_URI_SUFFIX}
#			${SRC_URI_KORG}/${PN}-manpages-${DOC_VER}${EXPORT_SUFFIX}.tar.${SRC_URI_SUFFIX}"
#			doc? (
#			${SRC_URI_KORG}/${PN}-htmldocs-${DOC_VER}${EXPORT_SUFFIX}.tar.${SRC_URI_SUFFIX}
#			)"
	[[ "${PV}" == *_rc* ]] || \
	KEYWORDS="~alpha ~amd64 ~arm ~arm64 ~hppa ~ia64 ~m68k ~mips ~ppc ~ppc64 ~riscv ~s390 ~sparc ~x86 ~x64-cygwin ~amd64-linux ~x86-linux ~ppc-macos ~x64-macos ~sparc-solaris ~sparc64-solaris ~x64-solaris ~x86-solaris"
fi
...
#	"${FILESDIR}"/git-2.21.0-quiet-submodules-testcase.patch
	...
	${FILESDIR}/git-2.35.1-git-instaweb-preserve-gitweb_config.perl-if-it-exist.patch
	${FILESDIR}/git-2.35.1-init-don-t-reset-core.filemode-on-git-new-workdirs.patch
)
...
#	#madhu FIXME;madhu 201203 - do this after git-r3_checkout
#	for i in  ${PN}-manpages-${DOC_VER}/man?; do ln -sv $i; done
